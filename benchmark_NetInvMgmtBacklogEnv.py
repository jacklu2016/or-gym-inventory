...
warnings.filterwarnings("ignore", category=DeprecationWarning) # Ignore potential NetworkX deprecations
...
ENV_CLASS_NAME = "NetInvMgmtBacklogEnv" # Choose Backlog or LostSales
...
try:
    ...
    NetInvManagementEnvClass = getattr(module, ENV_CLASS_NAME)
...
# --- Configuration ---
# Benchmark settings
N_EVAL_EPISODES = 20          # Fewer episodes for this complex env
RL_TRAINING_TIMESTEPS = 75000 # Reduced for quicker testing - INCREASE significantly!
SEED_OFFSET = 6000
FORCE_RETRAIN = False
COLLECT_STEP_DETAILS = False # Turn off by default for potentially large state/action
N_ENVS_TRAIN = 1 # Start with 1 env for stability with complex envs, increase later if needed

# <<<<<<<<<<<<<<<<< CHANGE: Paths for Network Env >>>>>>>>>>>>>>>>>
ENV_NAME_SHORT = "NetInvMgmt" # Changed prefix (use "NetInvMgmtLS" for lost sales)
...
# Override params like num_periods here if desired.
ENV_CONFIG = {
    'num_periods': 40, # Shorter than default 30? Let's use 40.
...
# <<<<<<<<<<<<<<<<< NEW Heuristic for Network Env >>>>>>>>>>>>>>>>>
class ConstantOrderAgent(BaseAgent):
    """ Heuristic: Orders a constant percentage of max capacity/bound for each link. """
    def __init__(self, order_fraction=0.1):
        # Orders order_fraction * action_space.high for each link
        super().__init__(name=f"ConstantOrder_{order_fraction*100:.0f}%")
        self.order_fraction = order_fraction
        self._action = None # Cache the action

    def get_action(self, observation: np.ndarray, env: gym.Env) -> np.ndarray:
        # Calculate action only once
        if self._action is None:
             high_bounds = env.action_space.high
             # Handle potential inf bounds if action space wasn't defined carefully
             high_bounds[high_bounds == np.inf] = 1000 # Replace inf with a heuristic large number
             self._action = (high_bounds * self.order_fraction).astype(env.action_space.dtype)
        return self._action


# SB3AgentWrapper reused (ensure action noise setup is robust)
class SB3AgentWrapper(BaseAgent):
    def __init__(self, model_class, policy="MlpPolicy", name="SB3_Agent", train_kwargs=None, model_kwargs=None):
        ...
        if model_class in [SAC, TD3, DDPG]:
             try:
                 # Need action dim and range for noise - create temp env
                 temp_env = NetInvManagementEnvClass(**ENV_CONFIG)
                 action_dim = temp_env.action_space.shape[0]
                 action_high = temp_env.action_space.high
                 action_high[action_high == np.inf] = 1000 # Handle inf bounds
                 action_range = action_high - temp_env.action_space.low # Assuming low is 0
                 temp_env.close()
    ...
    def train(self, env_config: dict, total_timesteps: int, save_path_prefix: str=""):
        ...
        def _create_env(rank: int, seed: int = 0):
            def _init():
                env = NetInvManagementEnvClass(**env_config); # Use correct env class
                # Check env maybe? Can be slow.
                # if rank == 0: try: check_env(env); except Exception as e_chk: print(f"WARN CheckEnv: {e_chk}")
        ...
        # if N_ENVS_TRAIN > 1: try: vec_env = SubprocVecEnv([_create_env(i, self.model_kwargs.get('seed', 0)) for i in range(N_ENVS_TRAIN)], start_method='fork'); except: print("SubprocVecEnv failed, using Dummy."); pass

        eval_env_callback = Monitor(NetInvManagementEnvClass(**env_config))
        eval_callback = EvalCallback(eval_env_callback, best_model_save_path=best_model_save_path, log_path=os.path.join(self.log_dir, "eval_logs"), eval_freq=max(5000, total_timesteps // 10), n_eval_episodes=3, deterministic=True, render=False) # Fewer eval eps
        try:
            model_seed = self.model_kwargs.get('seed', None)
            # Ensure observation space is float32 for SB3 models
            # NetInvMgmt already uses float32 observation space
            ...
            self.model.learn(total_timesteps=total_timesteps, callback=eval_callback, log_interval=100, **self.train_kwargs) # Log less often maybe
    ...
    def get_action(self, observation: np.ndarray, env: gym.Env) -> np.ndarray:
        ...
        # Observation should already be float32 from env
        action, _ = self.model.predict(observation, deterministic=True)
        # Action from model is float32, matches env action space
        return action.astype(env.action_space.dtype)


# --- Evaluation Function (Adapted for NetInvMgmt Internal State Access) ---
def evaluate_agent(agent: BaseAgent,
                   env_config: dict,
                   n_episodes: int,
                   seed_offset: int = 0,
                   # fixed_params not implemented yet for NetInvMgmt
                   collect_details: bool = False # Default False for complex env
                   ) -> Dict:
    """
    Evaluates an agent over n_episodes for NetInvManagement Env.
    Uses internal env DataFrames (X, D, S, U) to calculate metrics.
    """
    episode_summaries = []
    all_step_details = []
    total_eval_time = 0.0
    # <<<<<<<<<<<<<<<<< Use Correct Env Class >>>>>>>>>>>>>>>>>
    eval_env = NetInvManagementEnvClass(**env_config)
    # Access internal attributes needed for metrics (assumes no Monitor wrapper here)
    # If Monitor is used, need eval_env.env to access X, D, S, U etc.
    ...
    for i in range(n_episodes):
        ...
        try:
            ...
            episode_reward = 0.0
            episode_steps = 0
            # Use DataFrames to store step results for aggregation
            ...
            while not terminated and not truncated:
                t_current = eval_env.period # Period before step
                ...
                # --- NetInvMgmt Specific Metrics from DataFrames ---
                # Record values from *this completed step* t_current (results stored at index t_current)
                # Demand D[t]
                ep_demand_df.loc[t_current] = eval_env.D.loc[t_current, eval_env.retail_links]
                # Sales S[t] for retail links
                ep_sales_df.loc[t_current] = eval_env.S.loc[t_current, eval_env.retail_links]
                # Unfulfilled U[t+1] for retail links (backlog/lost sales from step t)
                ep_stockout_df.loc[t_current] = eval_env.U.loc[t_current + 1, eval_env.retail_links]
                # Ending Inventory X[t+1] for main nodes
                ep_end_inv_df.loc[t_current] = eval_env.X.loc[t_current + 1, eval_env.main_nodes]

                # Collect detailed step data if requested (use DFs for less memory)
                if collect_details:
                     step_data = {'step': episode_steps, 'reward': reward, 'action': action.tolist()}
                     # Add specific state elements if needed, carefully
            ...
            # Calculate episode operational metrics by summing/averaging DFs
            ...
            avg_ending_inv_all_nodes = ep_end_inv_df.mean().mean() # Avg inv per node per step
            ...
            ep_summary = {"Agent": agent.name, "Episode": i + 1, "TotalReward": episode_reward, "Steps": episode_steps, "Time": episode_time, "Seed": episode_seed,
                          "AvgServiceLevel": service_level_overall, # Overall service level
                          "TotalStockoutQty": total_stockout_all_retail, # Overall stockouts
                          "AvgEndingInv": avg_ending_inv_all_nodes, # Avg inventory across nodes/time
                          "Error": None}
...
def process_and_report_results(all_eval_results: List[Dict], agent_objects: Dict):
    ...
    # It correctly calculates summary stats and saves results using global RESULTS_DIR and ENV_NAME_SHORT
...
def plot_learning_curves(log_dirs: Dict[str, str], title: str = "RL Learning Curves"):
    # ... (Keep the function from benchmark_invmanagement.py) ...
    if not SB3_AVAILABLE: return
    ...
    for name, log_dir in log_dirs.items():
        ...
        try:
            ...
            if len(results['r']) > 0:
                ...
                if len(x) > 10: y = results.rolling(window=min(50, len(x)//2)).mean()['r'] # Wider smooth window
...
def plot_benchmark_results(df_summary: pd.DataFrame, df_raw_summary: pd.DataFrame):
    # ... (Keep the function from benchmark_invmanagement.py, use ENV_NAME_SHORT) ...
    ...
    if SB3_AVAILABLE:
        sb3_agents_def = [
            ...
            ("SAC-LowLR", SAC, {'model_kwargs': {'learning_rate': 1e-4}}),
             # LSTM Policies might be very relevant for network delays
             ("PPO-LSTM", PPO, {'policy': "MlpLstmPolicy"}),
             ("A2C-LSTM", A2C, {'policy': "MlpLstmPolicy"}),
             # Network size variations
             ("PPO-SmallNet", PPO, {'model_kwargs': {'policy_kwargs': dict(net_arch=dict(pi=[64, 64], vf=[64, 64]))}}), # Slightly larger for network
