# benchmark_netinvmgmt_ls_combined.py
...
try:
    ...
    potential_paths = [script_dir, os.path.join(script_dir, 'or-gym-inventory'),
    ...
    EnvClass = getattr(module, ENV_CLASS_NAME)  # Uses the changed class name
    print(f"Successfully imported {ENV_CLASS_NAME} from {ENV_MODULE_NAME}.")

except ImportError as e:
    print(f"Error importing {ENV_CLASS_NAME}: {e}"); sys.exit(1)
except Exception as e:
    print(f"Error during import: {e}"); sys.exit(1)

# --- Configuration ---
# Benchmark settings
N_EVAL_EPISODES = 10  # Reduced further for network env
RL_TRAINING_TIMESTEPS = 75000  # Reduced further for testing - INCREASE!
# <<<<<<<<<<<<<<<<< CHANGE: Seed Offset >>>>>>>>>>>>>>>>>
SEED_OFFSET = 12000  # Use different offset
FORCE_RETRAIN = False
COLLECT_STEP_DETAILS = False
N_ENVS_TRAIN_SB3 = 1
N_WORKERS_RLLIB = 1  # 0 means use driver only

# <<<<<<<<<<<<<<<<< CHANGE: Paths for NetInvMgmt Lost Sales >>>>>>>>>>>>>>>>>
...
LOG_DIR_SB3 = os.path.join(LOG_DIR, "SB3");
...
# The EnvClass itself ensures backlog=False
ENV_CONFIG = {
...
ENV_NAME_RLLIB = "netinvmgmt_ls_rllib_env-v0"  # Changed name
...
if RAY_AVAILABLE:
    try:
        register_env(ENV_NAME_RLLIB, env_creator); print(f"Registered '{ENV_NAME_RLLIB}' with RLlib.")
    except Exception as e_reg:
        print(f"Warning: Could not register env with RLlib: {e_reg}")


# --- Agent Definitions ---
# (BaseAgent, RandomAgent, ConstantOrderAgent, SB3AgentWrapper, RLlibAgentWrapper)
# Copy the *exact* class definitions from benchmark_netinvmgmt_combined.py here.
# They are designed to be reusable and adapt based on the specific environment instance.
...
class ConstantOrderAgent(BaseAgent):
    def __init__(self, order_fraction=0.1): super().__init__(
        name=f"ConstantOrder_{order_fraction * 100:.0f}%"); self.order_fraction = order_fraction; self._action = None

    def get_action(self, observation: np.ndarray, env: gym.Env) -> np.ndarray:
        if self._action is None: high = env.action_space.high.copy(); high[high == np.inf] = 1000; self._action = (
                    high * self.order_fraction).astype(env.action_space.dtype)
        return self._action


class SB3AgentWrapper(BaseAgent):
    # ... (Exact code from benchmark_netinvmgmt_combined.py) ...
    # Uses updated LOG_DIR_SB3, MODEL_DIR_SB3, EnvClass
    def __init__(self, model_class, policy="MlpPolicy", name="SB3_Agent", train_kwargs=None, model_kwargs=None):
        ...
        self.policy = policy;
        self.model = None
        self.train_kwargs = train_kwargs if train_kwargs else {};
        self.model_kwargs = model_kwargs if model_kwargs else {}
        self.log_dir = os.path.join(LOG_DIR_SB3, self.name);
        self.save_path = os.path.join(MODEL_DIR_SB3, f"{self.name}.zip")
        os.makedirs(os.path.dirname(self.log_dir), exist_ok=True);
        os.makedirs(os.path.dirname(self.save_path), exist_ok=True)
        if model_class in [SAC, TD3, DDPG]:
            try:
                t_env = EnvClass(**ENV_CONFIG); a_dim = t_env.action_space.shape[
                    0]; a_h = t_env.action_space.high.copy(); a_l = t_env.action_space.low.copy(); t_env.close(); a_h[
                    a_h == np.inf] = 1000; a_l[a_l == -np.inf] = 0; a_r = a_h - a_l; n_std = 0.1 * a_r / 2; n_std[
                    n_std <= 0] = 0.01; self.model_kwargs['action_noise'] = NormalActionNoise(mean=np.zeros(a_dim),
                                                                                              sigma=n_std); print(
                    f"SB3 Noise {name} std:{n_std.mean():.2f}")
            except Exception as e:
                print(f"Warn: SB3 noise setup fail {name}: {e}")

    def train(self, ec, tt, sp=""):
        if not SB3_AVAILABLE: print(f"Skip train {self.name}: SB3 missing"); return
        print(f"Train SB3 {self.name}...");
        st = time.time();
        tld = os.path.join(self.log_dir, "train_logs");
        os.makedirs(tld, exist_ok=True)
        sp = os.path.join(MODEL_DIR_SB3, f"{sp}{self.name}.zip");
        bmsp = os.path.join(MODEL_DIR_SB3, f"{sp}{self.name}_best");
        os.makedirs(bmsp, exist_ok=True)
        if not FORCE_RETRAIN and os.path.exists(sp): print(f"Load SB3 {sp}"); self.load(sp);
        if hasattr(self.model, '_total_timesteps') and self.model._total_timesteps >= tt: self.training_time = 0; print(
            "SB3 Model already trained."); return

        def _create_env(rank: int, seed: int = 0):
            def _init():
                env = EnvClass(**ec);
                env = Monitor(env, filename=os.path.join(tld, f"monitor_{rank}"));
                env.reset(seed=seed + rank);
                return env;

            return _init

        ve = DummyVecEnv([_create_env(0, self.model_kwargs.get('seed', 0))])
        ee = Monitor(EnvClass(**ec));
        ecb = EvalCallback(ee, best_model_save_path=bmsp, log_path=os.path.join(self.log_dir, "eval_logs"),
                           eval_freq=max(5000, tt // 10), n_eval_episodes=3, deterministic=True, render=False)
        try:
            ms = self.model_kwargs.get('seed', None);
            self.model = self.model_class(self.policy, ve, verbose=0, seed=ms, **self.model_kwargs);
            print(f"Start SB3 train {self.name} ({tt} steps)...")
            self.model.learn(total_timesteps=tt, callback=ecb, log_interval=100, **self.train_kwargs);
            self.training_time = time.time() - st;
            print(f"Train SB3 {self.name} done: {self.training_time:.2f}s.")
            self.model.save(sp);
            print(f"SB3 final model saved: {sp}")
            try:
                bmp = os.path.join(bmsp, "best_model.zip");
                if os.path.exists(bmp):
                    print(f"Load SB3 best: {bmp}..."); self.model = self.model_class.load(bmp)
                else:
                    print("SB3 best not found, use final.")
            except Exception as e:
                print(f"Warn: SB3 load best fail. Err: {e}")

        except Exception as e:
            print(f"!!! ERR SB3 train {self.name}: {e}");
            import traceback;
            traceback.print_exc();
            self.model = None;
            self.training_time = time.time() - st
...
def load(self, p):
    ...
    try:
        print(f"Load SB3 {self.name} from {p}"); self.model = self.model_class.load(p)
    except Exception as e:
        print(f"!!! ERR SB3 load {self.name}: {e}"); self.model = None
...
class RLlibAgentWrapper(BaseAgent):
    # ... (Exact code from benchmark_netinvmgmt_combined.py) ...
    # Uses updated LOG_DIR_RLLIB, MODEL_DIR_RLLIB, ENV_NAME_RLLIB
    def __init__(self, algo_name: str, config_updates: dict = None, name: str = None):
        ...
        self.log_dir = os.path.join(LOG_DIR_RLLIB, self.name);
        self.checkpoint_dir = os.path.join(MODEL_DIR_RLLIB, f"{self.name}_checkpoints");
    ...
    def _build_config(self, env_config):
        try:
            ...
            config = config.training(gamma=self.config_updates.get("gamma", 0.99),
                                     lr=self.config_updates.get("lr", 1e-4))
            ...
            print(f"!!! ERROR build RLlib config {self.name}: {e}"); raise

    def train(self, ec, tt, sp=""):
        if not RAY_AVAILABLE: print(f"Skip train {self.name}: Ray missing"); return
        print(f"Train RLlib {self.name}...");
        st = time.time();
        log_data = []
        last_chkpt = None;  # Find latest checkpoint dir if exists
        if os.path.exists(self.checkpoint_dir) and any(
            os.scandir(self.checkpoint_dir)): last_chkpt = ray.rllib.train.get_checkpoint_dir(self.checkpoint_dir)
        restored = False
        try:
            config = self._build_config(ec);
            self.algo = config.build()
            if not FORCE_RETRAIN and last_chkpt:
                print(f"Restore RLlib {self.name} from {last_chkpt}");
                self.algo.restore(last_chkpt);
                restored = True
                if self.algo.iteration * self.algo.config.train_batch_size >= tt: print(
                    "RLlib already trained."); self.training_time = 0; self.policy = self.algo.get_policy(); self.algo.stop(); return
            else:
                print(f"Start RLlib train {self.name} from scratch ({tt} steps)...")
            t_total = self.algo.iteration * self.algo.config.train_batch_size if restored else 0;
            it = self.algo.iteration if restored else 0
            while t_total < tt:
                it += 1;
                res = self.algo.train();
                t_total = res["timesteps_total"];
                mean_rw = res.get("episode_reward_mean", float('nan'));
                log_data.append({'iter': it, 'steps': t_total, 'reward': mean_rw})
                if it % 20 == 0: print(f"  RLlib {self.name} It:{it} St:{t_total}/{tt} Rw:{mean_rw:.1f}")
            self.training_time = time.time() - st;
            print(f"Train RLlib {self.name} done: {self.training_time:.2f}s.")
            cp = self.algo.save(self.checkpoint_dir);
            print(f"RLlib final checkpoint: {cp}");
            self.checkpoint_path = cp
            self.policy = self.algo.get_policy()
        except Exception as e:
            print(f"!!! ERR RLlib train {self.name}: {e}"); import \
                traceback; traceback.print_exc(); self.training_time = time.time() - st
        finally:
            if self.algo: self.algo.stop()
            if log_data: pd.DataFrame(log_data).to_csv(self.training_log_path, index=False); print(
                f"Saved RLlib log: {self.training_log_path}")

    def load(self, p):
        print(f"RLlib load {self.name}. Restore in train."); self.checkpoint_path = p

    def get_action(self, o, e):  # Assumes observation o is float32
        if self.policy is None and self.checkpoint_path:
            print(f"Policy {self.name} not loaded, try restore from {self.checkpoint_path}")
            try:
                cfg = self._build_config(ENV_CONFIG); temp_algo = cfg.build(); temp_algo.restore(
                    self.checkpoint_path); self.policy = temp_algo.get_policy(); temp_algo.stop(); print(
                    " Policy restored.")
            except Exception as e_restore:
                print(f" Restore failed: {e_restore}"); self.policy = None
        if self.policy is None: print(
            f"Warn: Policy {self.name} missing. Random."); a = e.action_space.sample(); return a.astype(
            e.action_space.dtype)
        try:
            action_data = self.policy.compute_single_action(o);
            action = action_data[0]
            return action.astype(e.action_space.dtype)  # Action is already float32
        except Exception as e:
            print(f"!!! ERR {self.name} get_action: {e}. Random."); a = e.action_space.sample(); return a.astype(
                e.action_space.dtype)


# --- Evaluation Function (Reusable) ---
# (evaluate_agent function remains the same as benchmark_netinvmgmt_combined.py)
def evaluate_agent(agent: BaseAgent,
                   env_config: dict,
                   n_episodes: int,
                   seed_offset: int = 0,
                   collect_details: bool = False) -> Dict:
    # ... (Keep the exact function from benchmark_netinvmgmt_combined.py) ...
    # It correctly uses EnvClass and calculates metrics via internal DFs
    episode_summaries = [];
    all_step_details = [];
    total_eval_time = 0.0
    ...
    for i in range(n_episodes):
        ...
        try:
            ...
            # Use internal env DataFrames to collect data efficiently
            ep_demand_df = pd.DataFrame(columns=eval_env.retail_links)
            ep_sales_df = pd.DataFrame(columns=eval_env.retail_links)
            ep_stockout_df = pd.DataFrame(columns=eval_env.retail_links)  # Stockout == Lost Sales in this env
            ...
            while not terminated and not truncated:
                ...
                if t_current < eval_env.num_periods:
                    ...
                    ep_stockout_df.loc[t_current] = eval_env.U.loc[
                        t_current + 1, eval_env.retail_links]  # U[t+1] = LS from period t
                    ep_end_inv_df.loc[t_current] = eval_env.X.loc[t_current + 1, eval_env.main_nodes]
                if collect_details: step_data = {'step': episode_steps, 'reward': reward,
                                                 'action': action.tolist()}; episode_step_details.append(step_data)
            end_time = time.perf_counter();
            episode_time = end_time - start_time;
            total_eval_time += episode_time
            total_demand = ep_demand_df.sum().sum();
            total_sales = ep_sales_df.sum().sum();
            total_stockout = ep_stockout_df.sum().sum();
            avg_end_inv = ep_end_inv_df.mean().mean()
            ...
            ep_summary = {"Agent": agent.name, "Episode": i + 1, "TotalReward": episode_reward, "Steps": episode_steps,
                          "Time": episode_time, "Seed": episode_seed, "AvgServiceLevel": service_lvl,
                          "TotalStockoutQty": total_stockout, "AvgEndingInv": avg_end_inv, "Error": None}
            ...
            if n_episodes <= 20 or (i + 1) % max(1, n_episodes // 5) == 0: print(
                f"  Ep {i + 1}/{n_episodes}: Rw={episode_reward:.0f}, SL={service_lvl:.1%}")
        except Exception as e:
            print(f"!!! ERR ep {i + 1} {agent.name}: {e}"); import \
    ...
    print(f"Eval done {agent.name}. Avg time: {avg_eval_time:.4f}s")
...
def process_and_report_results(all_eval_results: List[Dict], agent_objects: Dict):
    ...
    try:
        ...
        print(f"\nRaw summary saved: {raw_summary_path}");
            ...
            with open(details_path, 'w') as f:
                for i, agent_results in enumerate(all_eval_results):
                    ...
                    for ep_num, steps in enumerate(agent_results.get('details', [])):
                        for step_data in steps: step_data['agent'] = agent_name; step_data[
                            'episode'] = ep_num + 1; serializable_data = {
                            k: (v.tolist() if isinstance(v, np.ndarray) else v) for k, v in step_data.items()}; f.write(
...
def plot_learning_curves(sb3_log_dirs: Dict[str, str], rllib_log_dirs: Dict[str, str], title: str):
    ...
    if SB3_AVAILABLE:
        ...
        for name, log_dir in sb3_log_dirs.items():
            ...
            if not monitor_files: print(f"  Warn: No SB3 logs {name}"); continue; monitor_path = os.path.dirname(
                monitor_files[0])
            try:
                results = load_results(monitor_path)
                if len(results['r']) > 0:
                    x, y = ts2xy(results, 'timesteps'); y = \
                    results.rolling(window=max(10, len(x) // 20), min_periods=1).mean()['r'] if len(x) > 10 else results[
                        'r']; plt.plot(x, y, label=f"{name} (SB3)"); plotted = True
                else:
                    print(f"  Warn: SB3 Log {name} empty.")
    ...
    if RAY_AVAILABLE:
        ...
        for name, log_dir_base in rllib_log_dirs.items():
            ...
            if os.path.exists(rllib_log_file):
                try:
                    log_df = pd.read_csv(rllib_log_file).dropna(subset=['reward']);
                    if not log_df.empty: x = log_df['steps'].values; y = log_df['reward'].rolling(window=max(10, len(x) // 20),
                                                                                              min_periods=1).mean().values if len(
                    x) > 10 else log_df['reward'].values; min_len = min(len(x), len(y)); x = x[:min_len]; y = y[
                                                                                                              :min_len]; plt.plot(
                    x, y, label=f"{name} (RLlib)", linestyle='--'); plotted = True
                except Exception as e: print(f"  Error plot RLlib log {name}: {e}")
        else:
            print(f"  Warn: No RLlib log file {rllib_log_file}")
    if plotted:
        plt.legend(); plt.grid(True); plt.tight_layout(); p = os.path.join(RESULTS_DIR,
                                                                           f"{ENV_NAME_SHORT}_benchmark_learning_curves.png"); plt.savefig(
            p); print(f"Saved: {p}"); plt.close()
    else:
        print("Skip learning curves plot - no data.")
...
if __name__ == "__main__":
    ...
    if RAY_AVAILABLE:
        try:
            ray.init(ignore_reinit_error=True, num_cpus=N_WORKERS_RLLIB + 1)
        except Exception as e_ray:
            print(f"Warn: Ray init fail: {e_ray}. RLlib skipped."); RAY_AVAILABLE = False
    ...
    if RAY_AVAILABLE:
        ...
        for name, cfg_updates in rllib_agents_def:
            ...
            agents_to_run_defs.append(
                (rllib_name, RLlibAgentWrapper, {'algo_name': name, 'config_updates': cfg_updates, 'name': rllib_name}))
    else:
    ...
    if final_summary is not None:
        sb3_log_dirs = {name: agent.log_dir for name, agent in agent_objects.items() if
                        isinstance(agent, SB3AgentWrapper)}
        rllib_log_dirs = {name: agent.log_dir for name, agent in agent_objects.items() if
                          isinstance(agent, RLlibAgentWrapper)}
        if sb3_log_dirs or rllib_log_dirs:
            try:
                print("\nPlotting learning curves..."); plot_learning_curves(sb3_log_dirs, rllib_log_dirs,
